...
function initializeHeroSlider() { const slides = document.querySelectorAll('.hero-slider-container .hero-slide'); const indicators = document.querySelectorAll('.hero-slider-container .hero-indicator'); const totalSlides = slides.length; if (totalSlides === 0) return; let currentSlideIndex = 0; let slideInterval; const intervalTime = 10000; const showSlide = (index) => { currentSlideIndex = (index + totalSlides) % totalSlides; slides.forEach(slide => { const video = slide.querySelector('.hero-video'); if (video) video.pause(); }); const activeSlide = slides[currentSlideIndex]; const activeVideo = activeSlide.querySelector('.hero-video'); if (activeVideo) { activeVideo.currentTime = 0; activeVideo.play().catch(error => console.log("Video autoplay was prevented.", error)); } slides.forEach((s, i) => s.classList.toggle('active', i === currentSlideIndex)); indicators.forEach((i, idx) => i.classList.toggle('active', idx === currentSlideIndex)); startSlideShow(); }; const startSlideShow = () => { clearInterval(slideInterval); slideInterval = setInterval(() => showSlide(currentSlideIndex + 1), intervalTime); }; const heroContainer = document.querySelector('.hero-slider-container'); heroContainer?.addEventListener('mouseenter', () => clearInterval(slideInterval)); heroContainer?.addEventListener('mouseleave', startSlideShow); window.nextSlide = () => showSlide(currentSlideIndex + 1); window.previousSlide = () => showSlide(currentSlideIndex - 1); window.currentSlide = (index) => showSlide(index - 1); showSlide(0); }
